    ...
    public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, NoiseSettings noiseSettings, Vector2 sampleCenter) {
        ...
        for (int y = 0; y < mapHeight; y++) {
            for (int x = 0; x < mapWidth; x++) {
                ...
                for (int i = 0; i < noiseSettings.octaves; i++) {
                    float sampleX = (x - halfWidth + octaveOffsets[i].x) / noiseSettings.scale * frequncy;
                    float sampleY = (y - halfHeight + octaveOffsets[i].y) / noiseSettings.scale * frequncy;
                ...
                if(noiseSettings.normaliseMode == NormalizeMode.Global) {
                    float normaliazedHeight = (noiseMap [x, y] + 1)/(maxPossibleHeight/.9f);
                    noiseMap [x, y] = Mathf.Clamp(normaliazedHeight, 0, int.MaxValue);
                }
            }
        }
        if (noiseSettings.normaliseMode == NormalizeMode.Local) {
            for (int y = 0; y < mapHeight; y++) {
                for (int x = 0; x < mapWidth; x++) {
